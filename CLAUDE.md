...
|---|---|---|
| Средний балл за четверть | **взвешенное** среднее оценок: `Σ(оценка×вес)/Σ(вес)` | до сотых (`7.45`) |
| Годовая оценка по предмету | среднее средних баллов **непустых** четвертей | до целого 1–10, `0.5` вверх |

Вес задаёт тип работы (`Grade.kind`, таблица `GRADE_KINDS` в `grades.ts`):
контрольная весит 2, остальное — 1. Вес денормализуется в `Grade.weight` при
выставлении, чтобы правка таблицы весов не меняла историю. Считай средние только
через `weightedAverage`, не дублируй формулу.
