...
/**
 * Предохранитель «Сетки на четверть»: больше стольких дат-кандидатов за один
 * запуск не бывает при правильных границах четверти. Превышение почти наверняка
 * значит опечатку в границах (год вместо месяца). Константа общая для сервера
 * и формы (по прецеденту MAX_BULK_GRADES): предпросмотр обязан знать тот же
 * предел, что и действие, иначе он предлагал бы заведомо обречённый запуск.
 */
