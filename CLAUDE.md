...
| `PARENT` | только дневники СВОИХ детей (связь `ParentLink`), штамп «Ознакомлен», отключение своего Telegram | `/family` |

**Родитель читает ТОЛЬКО своих детей.** Любая функция, отдающая данные ученика
по `studentId`, обязана начинаться с `requireOwnChild(viewer, studentId)`
(`src/lib/family-guards.ts`) — единой двери с явным аллаулистом ролей. Отказ —
всегда **404 «Ученик не найден»**, а не 403: ответ «запрещено» подтверждал бы
перебирающему, что такой id существует. Связь и роль проверяются живым запросом
к базе — отвязка действует со следующего запроса. Записывать родитель может
только штампы `GradeAck` (снимок `seenValue`; «устарел» ВЫЧИСЛЯЕТСЯ как
`seenValue !== value`, при правке оценки штамп НЕ удаляется — это подпись),
отметку визита и отключение своего Telegram. `GRADE_EDITOR_ROLES` не расширяется.
Middleware держит аллаулист секций (`SECTION_ROLES` в `auth.config.ts`):
...
src/
  ...
  lib/
    ...
    family-guards.ts     ★ requireOwnChild — единая дверь к данным ученика (чужой = 404)
