 ...
 *  1. локальный `npm run db:seed` — очищает базу и заливает данные заново;
 *  2. `prisma/seed-if-empty.ts` в сборке Vercel — заливает только в пустую базу,
 *     поэтому повторные деплои ничего не портят.
 *
...
/**
 * Идентификаторы демо-данных фиксированы (`demo-student-1`, `demo-lesson-1-1-1`).
 * Так на них удобно ссылаться в проверках и документации, а повторный сид
 * не создаёт дубликатов. Записи, созданные пользователями, получают обычные cuid.
 */
